...
docker build --no-cache -t create-built-tiles:1.0.13 .
docker run -v ~/key.json:/tmp/keys/key.json -v ~/vm_data:/container/data -d --name create-built-tiles create-built-tiles:1.0.13
