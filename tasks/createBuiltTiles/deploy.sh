...
docker build --no-cache -t create-built-tiles:1.1.1 .
docker run -v ~/key.json:/tmp/keys/key.json -v ~/vm_data:/container/data -d --name create-built-tiles create-built-tiles:1.1.1
